...
import { ArrowLeft, Clock, CheckCircle, XCircle, Loader, ChevronDown, ChevronRight, Download, RefreshCw, ChevronRight as ChevronRightIcon } from 'lucide-react';
...
export const ExecutionVisualizer = () => {
        ...
        fetch(`http://localhost:3000/executions/${id}`)
            ...
            .then((data: Instance) => {
                ...
                definition.steps.forEach((step: any, index: number) => {
                    ...
                    const stepTasks = data.tasks.filter(t => t.taskId.startsWith(step.stepId));
    ...
    const totalDuration = formatDuration(instance.createdAt, instance.completedAt);
    const completedTasks = instance.tasks.filter(t => t.status === 'COMPLETED').length;
    const failedTasks = instance.tasks.filter(t => t.status === 'FAILED').length;
    const selectedNodeTasks = selectedNode ? instance.tasks.filter(t => t.taskId.startsWith(selectedNode)) : instance.tasks;
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Execution: {instance?.id.slice(0, 8)}...</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-6">
                        <div className="text-sm text-gray-600">
                            <span className="font-medium">Duration:</span> {instance && formatDuration(instance.createdAt, instance.completedAt)}
                        </div>
                        <div className="text-sm text-gray-600">
                            <span className="font-medium">Tasks:</span> {instance && `${instance.tasks.filter(t => t.status === 'COMPLETED').length}/${instance.tasks.length} completed`}
                            {instance && instance.tasks.filter(t => t.status === 'FAILED').length > 0 && <span className="text-red-600 ml-1">({instance.tasks.filter(t => t.status === 'FAILED').length} failed)</span>}
                        </div>
                                    ...
                                    <div key={task.id} className="p-4">
                                        ...
                                        >
                                            <div className="flex items-start gap-2 flex-1">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="text-xs text-gray-500 mt-1">ID: {task.taskId}</div>
                                        ...
                                        {isExpanded && (
                                            <div className="mt-3 space-y-3 text-xs">
                                                {task.startedAt && (
                                                    <div>
                                                        <div className="text-gray-500 font-medium">Started:</div>
                                                        <div className="text-gray-700">{new Date(task.startedAt).toLocaleString()}</div>
                                                    </div>
                                                )}
                                                {task.completedAt && (
                                                    <div>
                                                        <div className="text-gray-500 font-medium">Completed:</div>
                                                        <div className="text-gray-700">{new Date(task.completedAt).toLocaleString()}</div>
                                                    </div>
                                                )}
                                                <div>
