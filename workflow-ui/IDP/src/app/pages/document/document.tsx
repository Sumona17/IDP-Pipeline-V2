...
const DocumentUploaded: React.FC = () => {
  ...
  const handleDocumentClick = (doc: UploadedDocument) => {
    navigate(
      `/document-review/${encodeURIComponent(doc.extractedDataKey)}/${encodeURIComponent(doc.originalFileKey)}`,
    );
  };
  ...
  const getFileIcon = (filename: string): JSX.Element => {
    ...
    return (
      <svg
        className={`w-4 h-4 ${colors[ext ?? ""] ?? "text-gray-600"} flex-shrink-0`}
        fill="currentColor"
        viewBox="0 0 20 20"
      >
            ...
            <div className="overflow-y-auto overflow-x-hidden max-h-[calc(100vh-280px)]">
              <table className="w-full divide-y divide-gray-200 text-lg">
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    <th className="px-2 py-1.5 text-left text-xs font-medium text-gray-500 uppercase w-[30%]">
                      Document Name
                    </th>
                    <th className="px-2 py-1.5 text-left text-xs font-medium text-gray-500 uppercase w-[15%]">
                      Document Type
                    </th>
                    <th className="px-2 py-1.5 text-left text-xs font-medium text-gray-500 uppercase w-[20%]">
                      Customer Name
                    </th>
                    <th className="px-2 py-1.5 text-left text-xs font-medium text-gray-500 uppercase w-[10%]">
                      Progress Score
                    </th>
                    <th className="px-2 py-1.5 text-left text-xs font-medium text-gray-500 uppercase w-[15%]">
                      Created At
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <div className="w-full h-1.5 bg-gray-200 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: doc.fileProgress
                                    ? `${parseFloat(doc.fileProgress)}%`
                                    : "0%",
                                }}
                              />
                            </div>
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                      (page) => {
                        ...
                        if (
                          ...
                          return (
                            <span key={page} className="text-gray-400 px-0.5">
                              ...
                            </span>
                          );
                        return null;
