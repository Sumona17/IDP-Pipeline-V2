...
const MyQueue: React.FC = () => {
  ...
  const pendingApprovalCount = submissions.filter((s) =>
    ["PENDINGAPPROVAL"].includes(normalizeStatus(s.status)),
  ).length;
  ...
  const tabs = [
    ...
    { key: "pendingapproval", label: `Pending Approval (${pendingApprovalCount})` },
