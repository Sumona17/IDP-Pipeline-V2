...
import { fetchMyApprovalList } from "../../../services/approval-list";
...
const MyApproval: React.FC = () => {
  ...
  useEffect(() => {
    const loadSubmissions = async () => {
      try {
        setLoading(true);
        const data: any = await fetchMyApprovalList();
        setSubmissions(data);
        console.log("Fetched approvallist:", data);
      } catch (err) {
  ...
  const completedCount = submissions.filter((s) =>
    ["APPROVED"].includes(normalizeStatus(s.status)),
  ).length;
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, "approval.xlsx");
  };
