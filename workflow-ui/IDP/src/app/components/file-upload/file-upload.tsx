...
const UploadDrawer: React.FC<UploadDrawerProps> = ({ open, onClose }) => {
  ...
  const getStatusBadge = (status: UploadingFile['status'], progress: number) => {
    ...
    if (status === 'uploading') return (
      <span className="flex items-center gap-1 text-[10px] font-semibold text-cyan-600 bg-cyan-50 px-2 py-0.5 rounded-full">
        <div className="w-2.5 h-2.5 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin" />
        {progress}%
      </span>
    );
    return (
      <span className="text-[10px] font-medium text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">Pending</span>
    );
  };
  ...
  return (
    <>
      ...
      />
      <div
        className={`fixed top-0 right-0 h-full w-[460px] bg-white shadow-2xl z-50 flex flex-col transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : 'translate-x-full'}`}
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          <div
            ...
            className={`relative border-2 border-dashed rounded-xl p-6 text-center transition-all duration-200 group select-none ${
              isUploading
                ...
                : isDragging
                ? 'cursor-copy border-cyan-500 bg-cyan-50'
                : 'cursor-pointer border-gray-300 bg-gray-50 hover:border-cyan-400 hover:bg-cyan-50/40'
            }`}
          >
            <div className={`mx-auto w-12 h-12 rounded-full flex items-center justify-center mb-3 transition-colors ${isDragging ? 'bg-cyan-100' : 'bg-white shadow-sm group-hover:bg-cyan-50'}`}>
              <svg className={`w-6 h-6 transition-colors ${isDragging ? 'text-cyan-600' : 'text-gray-400 group-hover:text-cyan-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>
            </div>
            <p className={`text-sm font-semibold mb-1 ${isDragging ? 'text-cyan-700' : 'text-gray-700'}`}>
          ...
          {submissionResult && (
            <div className="flex items-start gap-2 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-emerald-600 mt-0.5 break-all">
              ...
              {uploadingFiles.map((f) => (
                <div
                  ...
                  className={`flex items-start gap-3 p-3 rounded-lg border transition-colors ${
                    ...
                    f.status === 'uploading' ? 'bg-cyan-50/60 border-cyan-200' :
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-1">
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        {f.status !== 'uploading' && (
                          <button onClick={() => removeFile(f.id)} className="p-0.5 text-gray-300 hover:text-gray-500 transition-colors rounded">
                    ...
                    {(f.status === 'uploading' || f.status === 'done') && (
                      <div className="mt-2 w-full h-1 bg-gray-200 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-300 ${f.status === 'done' ? 'bg-emerald-500' : 'bg-cyan-500'}`}
          ...
          {uploadingFiles.length === 0 && (
            <div className="text-center py-8 text-gray-400">
              <svg className="mx-auto w-8 h-8 mb-2 text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex-shrink-0 border-t border-gray-200 px-5 py-3.5 bg-gray-50 flex items-center justify-between gap-3">
          <button
            onClick={handleClose}
            disabled={isUploading}
            className="px-4 py-2 text-xs font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Close
          </button>
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isUploading}
              className="px-3 py-2 text-xs font-medium text-cyan-700 bg-cyan-50 border border-cyan-200 rounded-lg hover:bg-cyan-100 transition-colors flex items-center gap-1.5 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              className="px-4 py-2 text-xs font-semibold bg-cyan-600 hover:bg-cyan-700 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed text-white rounded-lg transition-colors flex items-center gap-1.5"
            >
              {isUploading ? (
                <>
