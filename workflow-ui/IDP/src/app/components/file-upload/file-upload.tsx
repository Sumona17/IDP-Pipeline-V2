...
const UploadDrawer: React.FC<UploadDrawerProps> = ({
  ...
  const getFileIcon = (filename: string): JSX.Element => {
    ...
    return (
      <svg
        className={`w-4 h-4 ${colors[ext ?? ""] ?? "text-gray-500"} flex-shrink-0`}
        fill="currentColor"
        viewBox="0 0 20 20"
      >
      ...
      const { submissionId: sid } = await batchUploadFiles(
        ...
        abortControllersRef.current,
        {
          ...
          onFileProgress: (fileName, percent) => {
            setUploadingFiles((prev) =>
              prev.map((f) =>
                f.file.name === fileName ? { ...f, progress: percent } : f,
              ),
            );
          },
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          <div
            ...
            onDrop={handleDrop}
            onClick={() =>
              !isUploading && !submissionResult && fileInputRef.current?.click()
            }
          ...
          >
            <div
              className={`mx-auto w-12 h-12 rounded-full flex items-center justify-center mb-3 transition-colors ${
                isDragging
                  ? "bg-[#E6DAFF]"
                  : "bg-white shadow-sm group-hover:bg-[#E6DAFF]"
              }`}
            >
              <svg
                className={`w-6 h-6 transition-colors ${
                  isDragging
                    ? "text-[#3C20F6]"
                    : "text-gray-400 group-hover:text-[#3C20F6]"
                }`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
            ...
            <p
              className={`text-sm font-semibold mb-1 ${isDragging ? "text-[#3C20F6]" : "text-gray-700"}`}
            >
              {isDragging
            ...
            <div className="flex items-start gap-2 p-3 bg-green-50 border border-green-200 rounded-xl">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-green-600 mt-0.5 break-all">
                  Submission ID:{" "}
                  <span className="font-mono">{submissionResult}</span>
                </p>
              </div>
            </div>
          )}
